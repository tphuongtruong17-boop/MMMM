...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!name || !symbol || !supply) return res.status(400).json({ error: 'Missing required fields' });

    const MNEMONIC = process.env.MNEMONIC;
    if (!MNEMONIC) return res.status(500).json({ error: 'Server not configured' });
    ...
    const deployment = await factory.signDeployment({
      ...
      network: NETWORK, feeRate: 10,
      priorityFee: 0n, gasSatFee: BigInt(20000),
      bytecode: wasmBytes, calldata: new Uint8Array(calldata),
      challenge,
    ...
    if (!funding.success) throw new Error('Funding TX failed: ' + funding.error);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!reveal.success) throw new Error('Reveal TX failed: ' + reveal.error);
